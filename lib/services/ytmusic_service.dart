...
/// Créditos OFICIALES de una pista: secciones "Rol: nombres" del diálogo
/// "Song credits" de YT Music y/o del texto auto-generado de la descripción.
/// Solo se acepta la descripción que trae el marcador de auto-generado; las
/// descripciones custom del canal NO son créditos.
class YtmTrackCredits {
  ...
  /// Secciones de crédito, p. ej. "Performed by" → [Bad Bunny, ...].
  final List<YtmCreditSection> sections;
  final String? album;
  final String? distributor;

  bool get isEmpty => sections.isEmpty && album == null && distributor == null;
}

/// Una sección de créditos: rol ("Written by") y personas acreditadas.
class YtmCreditSection {
  const YtmCreditSection({required this.role, required this.names});
...
class YtMusicService {
  ...
  /// Official credits for a track, two sources in order:
  /// 1. `browse MPTC<videoId>` — YT Music's own "Song credits" dialog
  ///    (structured role sections, present even on custom uploads).
  /// 2. `next` description panel — only when it carries the "Auto-generated
  ///    by YouTube" marker; custom channel descriptions are NOT credits.
  Future<YtmTrackCredits?> fetchTrackCredits(String videoId) async {
    final id = videoId.trim();
    if (id.isEmpty) return null;
    return await fetchTrackCreditsDialog(id) ??
        await fetchTrackCreditsDescription(id);
  }

  /// Source 1: the dedicated credits dialog (`browse MPTC<id>`, WEB_REMIX).
  Future<YtmTrackCredits?> fetchTrackCreditsDialog(String videoId) async {
    final body = jsonEncode({
      'context': _context(),
      'browseId': 'MPTC${videoId.trim()}',
    });
    http.Response res;
    try {
      res = await _client
    ...
    return parseTrackCreditsDialog(data);
  }

  /// Source 2: auto-generated credits from the `next` (WEB) description.
  Future<YtmTrackCredits?> fetchTrackCreditsDescription(String videoId) async {
    final body = jsonEncode({
      'context': {
        'client': {
          'clientName': 'WEB',
    ...
    return parseTrackCreditsDescription(data);
  }

  /// Source 1 parser: `browse MPTC<id>` returns the "Song credits" dialog
  /// with structured sections ("Performed by", "Written by"…) — every name
  /// run is one credit. Album/distributor are not part of this dialog.
  static YtmTrackCredits? parseTrackCreditsDialog(Object? node) {
    final sections = <String, List<String>>{};
    void walk(Object? n) {
      if (n is Map) {
        final dialog = n['dismissableDialogRenderer'] as Map?;
        final dialogSections = dialog?['sections'] as List?;
        if (dialogSections != null) {
          for (final s in dialogSections) {
            if (s is! Map) continue;
            final sec = s['dismissableDialogContentSectionRenderer'] as Map?;
            final role = ((sec?['title'] as Map?)?['runs'] as List?)
                ?.map((r) => (r as Map)['text'])
                .whereType<String>()
                .join()
                .trim();
            final names = ((sec?['subtitle'] as Map?)?['runs'] as List?)
                ?.map((r) => ((r as Map)['text'] as String?) ?? '')
                .map((t) => t.trim())
                .where((t) => t.isNotEmpty)
                .toList();
            if (role == null || role.isEmpty || names == null) continue;
            sections.putIfAbsent(role, () => []).addAll(names);
          }
          return;
        }
        n.values.forEach(walk);
    ...
    final credits = YtmTrackCredits(
      sections: [
        for (final e in sections.entries)
          YtmCreditSection(role: e.key, names: e.value.toSet().toList()),
      ],
    );
    return credits.isEmpty ? null : credits;
  }

  /// Source 2 parser: the `next` description panel, ACCEPTED ONLY when the
  /// text carries the "Auto-generated by YouTube" marker — custom channel
  /// descriptions (tour links, track listings, socials) are not credits.
  static YtmTrackCredits? parseTrackCreditsDescription(Object? node) {
    String? text;
    void walk(Object? n) {
      if (text != null) return;
      if (n is Map) {
    ...
    if (!RegExp('auto-generated by', caseSensitive: false).hasMatch(raw)) {
      return null;
    }
    ...
    String? album;
    final sections = <String, List<String>>{};
    void addNames(String role, List<String> names) {
      if (names.isNotEmpty) sections.putIfAbsent(role, () => []).addAll(names);
    }
    ...
    for (final line in raw.split('\n')) {
      ...
      if (l.contains(' \u00B7 ')) {
        final parts = l
            ...
            .toList();
        if (parts.length > 1) {
          // First slot is the track title — drop it, keep the people.
          addNames('Performed by', parts.sublist(1));
        }
        continue;
      }
      final idx = l.indexOf(':');
